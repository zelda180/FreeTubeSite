...
function getFileData($_XML_DATA){
	...
	if(isset($_XML_DATA[0]['child'][2]['child'])){
		...
		for($i = 0; $i < $num_files; $i++){
			...
			if(isset($_XML_DATA[0]['child'][2]['child'][$i]['child'][4]['name']) && isset($_XML_DATA[0]['child'][2]['child'][$i]['child'][4]['content'])){
				$file_info->setFileInfo($_XML_DATA[0]['child'][2]['child'][$i]['child'][4]['name'], $_XML_DATA[0]['child'][2]['child'][$i]['child'][4]['content']);
			}

			if(strlen($file_info->slot) > 0){ $_file_data[$file_info->slot] = $file_info; }
		}
	}
...
function getFileDataTable($_FILE_DATA, $_CONFIG_DATA, $_POST_DATA){
	...
	$file_list .= "   <tr><th>UPLOADED FILE NAME</th><th>UPLOADED FILE SIZE</th></tr>\n";
